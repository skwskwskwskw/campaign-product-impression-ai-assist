...
| Metric | Description |
|--------|-------------|
| `spend_fair` | Product-level spend (fair allocation from input metrics) |
| `spend_lead_only` | Ad spend redistributed only to lead products |
| `gross_profit_fair` | Product-level gross profit (fair allocation from input metrics) |
| `gross_profit_lead_only` | Gross profit redistributed only to lead products |
...
```
`match_stage` is included when generated by the ad-product matcher.
...
gp_lead, gp_halo, spillover_share, gp_per_spend_total, gp_per_1k_impr_total
```
